...
export default (telemetry) => {
  telemetry.job.on(eventNames.cancelled, (job) => {
  ...
  telemetry.job.on(eventNames.delivered, (job) => {
    const text = `Job delivered, ${currency()}${job.revenue.toLocaleString()} / ${
      job.earnedXP
    }XP`;
    const data = {
      job: job
  ...
  telemetry.job.on(eventNames.finished, (job) => {
    const data = {
      job: job
    };
    log(eventNames.finished, data, 'Job finished');
  });
  telemetry.job.on(eventNames.started, (job) => {
    const data = {
      job: job
    };
