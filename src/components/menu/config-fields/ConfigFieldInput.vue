  ...
  >
    ...
    >
      ...
      <input
        ...
        v-bind="listProp"
      />
      <datalist v-if="vals.length > 0" :id="id + '_' + 'data-list'">
        <option
          v-for="value in vals"
          :key="value.value"
      ...
      <button class="input-zone valid" :disabled="disabled" @click="set(val)">
...
export default {
  ...
  computed: {
    listProp() {
      return this.vals.length > 0 ? { list: this.id + '_' + 'data-list' } : {};
    }
  }
};
</script>
