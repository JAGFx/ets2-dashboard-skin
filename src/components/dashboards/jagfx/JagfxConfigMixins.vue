...
export default {
  ...
  mounted() {
    ...
    this.elements['middle-t'] = configMiddleTop.slice(
      0,
      this.maxElements['middle-t']
    );
  ...
  methods: {
    $elementIsEnabled: function (side, element, options) {
      ...
      if (options !== undefined) {
        ...
        if (navElmSide === 'right') {
          if (
            indexElement + 1 <= maxMiddleElement / 2 ||
            indexElement + 1 > maxMiddleElement
          )
            return false;
        }
      }

      if (!this.$store.getters['config/enabled'](element)) return false;
